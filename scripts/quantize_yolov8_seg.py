...
class YoloSegDataReader(CalibrationDataReader):
    ...
    def preprocess(self, image_path):
        # YOLOv8-Seg expects RGB, 0-1 float32, (1, 3, 640, 640)
...
def export_to_onnx(model_name='yolov8s-seg.pt'):
...
def quantize_model(onnx_path, calibration_data_dir, output_dir='models'):
    ...
    dr = YoloSegDataReader(calibration_data_dir, input_name, input_shape, size=128)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Export and quantize YOLOv8-Seg for NPU')
    parser.add_argument('--model', type=str, default='yolov8s-seg.pt', help='YOLO segmentation model name')
    ...
    if isinstance(onnx_file, str):
        ...
        onnx_path = 'yolov8s-seg.onnx'

    quantize_model(onnx_path, args.data_dir, args.output_dir)
    print("\n✅ YOLOv8-Seg quantization complete!")
    print(f"   Run inference with: python inference/npu_segmentation_inference.py")
